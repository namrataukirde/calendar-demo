module service
  class TaskBuilder
    ...
    def initialize(event_list_response)
      self.event_list = event_list_response
      self.task = nil
      self.event = nil
    end
    ...
    def create_or_update
      event_list.each do |event|
        @event = event
        @task = Task.where(google_event_id: event["id"]).first_or_create
        assign_field_values
    ...
    def assign_date_field_values
      GOOGLE_CALENDAR_API_DATE_TIME_FIELD_TO_TASK_MAPPING.each do |gcal_date_field, task_db_column|
        task.send("#{task_db_column}=", event[gcal_date_field]["dateTime"]) unless event[gcal_date_field]["dateTime"].blank?
      end
    end
  end
end
